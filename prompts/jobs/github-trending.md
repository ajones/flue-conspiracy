...
1. Ensure the dedup history file `.github-trending` exists in your workspace.
...
2. Fetch the GitHub Trending page: https://github.com/trending
   - Use `web_fetch` to retrieve it. If the content looks empty, boilerplate, or JS-rendered with no real data, retry with the Playwright scraper skill (`skills/playwright-scraper-skill`, `node scripts/playwright-simple.js "https://github.com/trending"`).
...
6. After composing the message (and before or immediately after delivering it), append a new line to `.github-trending` in your workspace in the format `<repo url>, <short description>` — using the same description you put in the message. Preserve all existing lines — only add the new one at the end.
