...
2. If the file does not exist, stop immediately and output exactly `NO_REPLY`.
3. If the file exists but cannot be parsed, stop immediately and output exactly `NO_REPLY`.
...
6. Look for `lastRead`.
7. If `lastRead` is missing, output exactly `NO_REPLY`.
