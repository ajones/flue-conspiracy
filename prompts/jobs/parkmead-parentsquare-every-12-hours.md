...
Task:
1. Check Gmail for unread emails from ParentSquare / Parkmead / Walnut Creek School District:
   - Use the `gws` CLI to list matching messages:
     ```bash
     gws gmail users messages list --params '{"userId": "me", "maxResults": 20, "q": "is:unread (from:parentsquare.com OR from:\"Parkmead Elementary\" OR from:\"Walnut Creek School District\")"}'
     ```
![[components/gws-gmail.md#fetch]]
2. For each new ParentSquare email, read the content and extract the key, actionable information, including (when present):
   - Event name/title
   - Date and time
   - Location
...
3. Update the Apple Note named "Parkmead Happenings" in the shared Apple Notes folder "Shared with Raven" with the extracted information.
   ...
   - Organize the note visually (as rendered in Apple Notes) in this order:
     ...
     5) **HEALTH & DISTRICT NOTICES**: health alerts and district-wide notices.
![[components/gws-gmail.md#apple-notes-rules]]
   - Add a short source reference (e.g., "(ParentSquare, <subject>, <date>)").
4. Be idempotent: avoid re-adding the same email/event if it's already been processed.
5. ![[components/gws-gmail.md#archive]]
6. At the very end of the run, perform a cleanup pass over **all** Parkmead/ParentSquare/Walnut Creek School District messages still in the Inbox:
   - Query for any remaining read messages in the Inbox:
     ```bash
     gws gmail users messages list --params '{"userId": "me", "maxResults": 50, "q": "in:inbox -is:unread (from:parentsquare.com OR from:\"Parkmead Elementary\" OR from:\"Walnut Creek School District\")"}'
     ```
