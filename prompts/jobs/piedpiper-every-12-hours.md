...
1. Check Gmail for unread emails from Pied Piper Preschool or Konstella:
   - Use the `gws` CLI to list matching messages:
     ```bash
     gws gmail users messages list --params '{"userId": "me", "maxResults": 20, "q": "is:unread (from:piedpiperpreschool.com OR from:konstella.com OR subject:\"Pied Piper\")"}'
     ```
...
3. Update the Apple Note named "Pied Piper Happenings" in the shared Apple Notes folder "Shared with Raven" with the extracted information.
...
![[components/gws-gmail.md#apple-notes-rules]]
   - Add a short source reference (e.g., "(Konstella, <subject>, <date>)" or "(piedpiperpreschool.com, <subject>, <date>)").
4. Be idempotent: avoid re-adding the same email/event if it's already been processed.
5. ![[components/gws-gmail.md#archive]]
6. At the very end of the run, perform a cleanup pass over **all** Pied Piper / Konstella messages still in the Inbox:
   - Query for any remaining read messages in the Inbox:
     ```bash
     gws gmail users messages list --params '{"userId": "me", "maxResults": 50, "q": "in:inbox -is:unread (from:piedpiperpreschool.com OR from:konstella.com OR subject:\"Pied Piper\")"}'
     ```
