#!/usr/bin/env node
/**
 * Cancel a booked BJJ class for a student on the gymdesk portal.
 *
 * Usage:
 ...
 * --date: YYYY-MM-DD of the class to cancel (must be in the displayed week; use --weeks-ahead to navigate forward)
 *
 * Exits 0 on success, 1 on failure.
 * Output: { "ok": true, "canceled": true, "title": "...", "date": "..." }
 */
...
async function main() {
  ...
  try {
    ...
    if (!target.booked) {
      out({ ok: true, canceled: true, alreadyCanceled: true, title: target.title, date: target.date });
      return;
    }
    if (!target.canCancel) {
      out({ ok: false, error: `Booking on ${date} can no longer be canceled` });
      process.exit(1);
    }

    // open the detail panel for this event and click "Cancel Booking"
    await page.click(`[data-test-id="schedule-event"][data-event-id="${target.eventId}"][attr-date="${date}"]`);
    await page.waitForTimeout(1500);
    await page.click('[data-test-id="schedule-event-details"] .details:visible .buttons a.button.cancel-booking');
    await page.waitForTimeout(1500);

    // confirm the cancel popup
    await page.click('.confirmation-popup:visible .buttons-container a.red-button.cancel');
    await page.waitForTimeout(2000);

    out({ ok: true, canceled: true, title: target.title, date: target.date });
  } catch (e) {
