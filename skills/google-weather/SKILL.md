...
metadata: {"clawdbot":{"emoji":"🌤️","primaryEnv":"GOOGLE_API_KEY","secondaryEnv":["GOOGLE_WEATHER_API_KEY","GOOGLE_MAPS_API_KEY","GOOGLE_APPLICATION_CREDENTIALS"]}}
allowed-tools: [exec]
---
...
bash skills/google-weather/lib/weather.sh current "New York"
bash skills/google-weather/lib/weather.sh current "London"
bash skills/google-weather/lib/weather.sh current "Sydney"

# 24h Forecast
bash skills/google-weather/lib/weather.sh forecast "Tel Aviv"

# Raw JSON data
bash skills/google-weather/lib/weather.sh json "Paris"
```
...
> Alternatively, if `GOOGLE_APPLICATION_CREDENTIALS` (a service account) is set and no API key is configured, the skill falls back to using that for OAuth-authenticated Weather API requests. Note that geocoding still requires an API key — without one, location lookups fall back to OpenStreetMap Nominatim.
...
```bash
# Hebrew output
bash skills/google-weather/lib/weather.sh current "Tel Aviv"
# Output: בהיר ☀️ 19°C...
```
